...
use crate::db::{
    ...
    mark_running, storage_state_path,
};

use super::payload::session_payload_async;
use super::types::{
    CreateSessionReq, LaunchSessionReq, PlatformReq, SessionCheckResult, SessionIdReq,
    SessionLaunchResult, StoredCookie,
};
...
pub(super) fn register(app: &mut BuilderApp) {
    ...
    app.route("sessions.delete", move |facade, req: SessionIdReq| {
        ...
        async move {
            ...
            if session.active_run_count > 0 {
                let payload =
                    session_payload_async(&db, &sessions_dir, &req.session_id, json!({})).await?;
                let _ = facade
    ...
    app.route("sessions.check", move |facade, req: SessionIdReq| {
        ...
        async move {
            ...
            let payload = session_payload_async(&db, &sessions_dir, &req.session_id, json!({})).await?;
            let value = facade
